...
const allPossibleSlices = <T>(_arr: T[], standin: T) => {
	...
	return arr.map((_, i) =>
		arr.map((_, k) => k).filter((k) => k > i).map((k) => [i, k] as [number, number])
	).flat();
};
...
const makeDict = (lines: Line[]) =>
	(referenceLines: string[]) => {
		...
		lines.forEach((line, i) => {
			...
			slices.map(slice => line.slice(slice[0], slice[1])).filter((slice) => isGood(slice))
				.forEach((slice) => {
