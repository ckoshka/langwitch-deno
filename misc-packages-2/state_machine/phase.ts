...
>(
	...
	return {
		...
		afterF: <Effs2 extends FnRecord, DerivedEffs2 extends FnRecord, NewOut>(
			...
			>({
				...phase,
				fn: (input: Message<MessageInput, StateType>) =>
					phase.fn(input).chain(fn),
			});
		},
		beforeF: <Effs2 extends FnRecord, DerivedEffs2 extends FnRecord>(
			fn: (
				input: Message<MessageInput, StateType>,
			) => Free<Message<MessageInput, StateType>, Effs2, DerivedEffs2>,
		) => {
			return Phase<
				Out,
				MessageInput,
		...
		after: <NewOut>(
			...
			return Phase<NewOut, MessageInput, StateType, Effs, DerivedEffs>({
				...phase,
				fn: (input: Message<MessageInput, StateType>) =>
					phase.fn(input).map(fn),
			});
		},
		before: (
			fn: (
				input: Message<MessageInput, StateType>,
			) => Message<MessageInput, StateType>,
		) => {
